...
@section('content')
    @php
        ...
        $intransitValue = fn($name) => optional($report->materialIntransits->firstWhere('material_name', $name))
            ->quantity;
    @endphp
    ...
    <div class="page-card">
        ...
        <p class="page-card-subtitle">
            Perbarui data operasional harian GP Dumai. Setelah disimpan, data pada dashboard dan database MySQL akan ikut
            berubah.
        </p>
    </div>
        ...
        <div class="form-section">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="date" name="report_date" value="{{ old('report_date', $report->report_date) }}"
                        required>
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="cement_mill_note"
                        value="{{ old('cement_mill_note', $report->cement_mill_note) }}"
                        placeholder="Contoh: Operasional lancar">
                </div>

                <div class="form-group">
                    <label>Feed</label>
                    <input type="number" step="0.01" min="0" name="feed"
                        value="{{ old('feed', $report->feed) }}" placeholder="0.00">
                </div>

                <div class="form-group">
                    <label>Blaine</label>
                    <input type="number" step="0.01" min="0" name="blaine"
                        value="{{ old('blaine', $report->blaine) }}" placeholder="0.00">
                </div>

                <div class="form-group">
                    <label>Sieving</label>
                    <input type="number" step="0.01" min="0" name="sieving"
                        value="{{ old('sieving', $report->sieving) }}" placeholder="0.00">
                </div>

                <div class="form-group">
                    <label>Produksi Semen</label>
                    <input type="text" inputmode="decimal" name="production_cm"
                        value="{{ old('production_cm', $report->production_cm ? number_format($report->production_cm, 2, ',', '.') : '') }}"
                ...
                <div class="form-group">
                    ...
                    <input type="text" inputmode="decimal" name="production_ship"
                        value="{{ old('production_ship', $report->production_ship ? number_format($report->production_ship, 2, ',', '.') : '') }}"
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0" name="running_hours"
                        value="{{ old('running_hours', $report->running_hours) }}" placeholder="0.00">
                </div>

                <div class="form-group">
                    <label>Clinker Factor</label>
                    <input type="number" step="0.01" min="0" name="clinker_factor"
                        value="{{ old('clinker_factor', $report->clinker_factor) }}" placeholder="0.00">
                </div>

                <div class="form-group">
                    <label>Silo Semen</label>
                    <input type="number" step="0.01" min="0" name="silo_semen"
                        value="{{ old('silo_semen', $report->silo_semen) }}" placeholder="0.00">
                </div>
            </div>
        </div>
                ...
                @foreach (['Klinker', 'Gypsum Natural', 'Gypsum Purified', 'Pozzolan', 'Wet Fly Ash', 'Dry Fly Ash', 'Limestone', 'Solar'] as $material)
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" min="0" name="stocks[{{ $material }}]"
                            value="{{ old('stocks.' . $material, $stockValue($material)) }}" placeholder="0.00">
                    </div>
                @endforeach
            </div>
        </div>
                ...
                @foreach (['Klinker', 'Gypsum Natural', 'Gypsum Purified', 'Pozzolan', 'Wet Fly Ash', 'Dry Fly Ash', 'Limestone', 'Semen Curah', 'Solar'] as $material)
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" min="0" name="receipts[{{ $material }}]"
                            value="{{ old('receipts.' . $material, $receiptValue($material)) }}" placeholder="0.00">
                    </div>
                @endforeach
            </div>
        </div>
                ...
                @foreach (['Semen', 'Klinker', 'Limestone', 'Gypsum', 'Pozzolan', 'Fly Ash Dry SDS', 'Fly Ash Dry ESM', 'Fly Ash Dry IK', 'Fly Ash Dry SDO', 'Fly Ash Wet Tenayan', 'Fly Ash Wet RAPP'] as $material)
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" inputmode="decimal" name="intransits[{{ $material }}]"
                            value="{{ old('intransits.' . $material, $value ? number_format($value, 2, ',', '.') : '') }}"
                            placeholder="0,00">
                    </div>
                @endforeach
            </div>
                ...
                @foreach (['Klinker', 'Gypsum Natural', 'Gypsum Purified', 'Pozzolan', 'Wet Fly Ash', 'Dry Fly Ash', 'Limestone', 'Solar', 'Gas'] as $material)
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" min="0" name="usages[{{ $material }}]"
                            value="{{ old('usages.' . $material, $usageValue($material)) }}" placeholder="0.00">
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0" name="production_packer"
                        value="{{ old('production_packer', $report->production_packer) }}" placeholder="0.00">
                </div>
            </div>
        </div>
                ...
                @foreach (['PCC 40 Kg', 'PCC 50 Kg', 'OPC 50 Kg', 'Big Bag'] as $bag)
                    <div class="form-group">
                        ...
                        <input type="number" step="1" min="0" name="bags[{{ $bag }}]"
                            value="{{ old('bags.' . $bag, $bagValue($bag)) }}" placeholder="0">
                    </div>
                @endforeach
            </div>
        </div>
